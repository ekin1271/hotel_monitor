...
async function scrapePage(browser, targetUrl, checkIn) {
  ...
  catch(e) { console.log(`  Timeout, devam ediliyor...`); }

  try { await page.waitForSelector('li.s8.i_t1', { timeout: 30000 }); console.log('    Ilk teklif elemanlari yuklendi.'); } 
  catch(e) { console.log('    Uyari: li.s8.i_t1 bulunamadi, devam ediliyor...'); }
  ...
  const results = await page.evaluate((agencyRulesStr) => {
    ...
    for (const tr of allRows) {
      ...
      // Artık tarihe bakılmıyor
      const matchedLi = agencyLis[0];
...
function loadState() { if (fs.existsSync(STATE_FILE)) return JSON.parse(fs.readFileSync(STATE_FILE, 'utf8')); return {}; }
...
async function main() {
  ...
  } else { console.log('Uyari yok, Peninsula onde veya durum degismedi.'); }
}
